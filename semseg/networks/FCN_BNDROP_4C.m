function [net, info] = FCN_BNDROP_4C(imdb, netF, inpt, varargin)

	% some common options
	trainer = @cnn_train_dag_seg_adv;
	opts.train.fcn_visualize = @visualize_segmentation_4C;
...
% defining batch policies 
     ...
     bopts_train.balanceStruct  = [1, 2; 2, 4; 12, 4];
     ...
     opts.train.classesNames = {'sky', 'road', 'no-nav', 'obstacle'};
     colorMap  = (1/255)*[		    
					    128 128 128
					    128 64 128
					    0 0 192
...
function inputs = getBatch(opts, imdb, batch)
	...
	labelsX = imdb.images.labels(:, :, :, batch) ;
	L = labelsX(:,:,1,:);
	W = labelsX(:,:,2,:);
	...
	labels(L == 1) = 1;			W2(L == 1) 		= W(L == 1);
	labels(L == 3) = 2;			W2(L == 3) 		= W(L == 3);
	labels(L == 4) = 3;			W2(L == 4) 		= 30.0;
	labels(L == 2 | L > 4) = 4;		W2(L == 2 | L > 4) 	= 5.0;
	labels = cat(3, labels, W2);
