function [net, info] = FCN_MULTIRES(imdb, netF, inpt, varargin)
...
% defining batch policies 
     ...
     bopts_train.balanceStruct  = [2, 15];
     bopts_train.maxNumBatches  = 50;
     ...
     bopts_val.imdb           = imdb;
     bopts_val.balanceStruct  = [3, 5];
     bopts_val.maxNumBatches  = 5;
...
%	opts.train.classesNames = {'sky', 'building', 'road', 'sidewalk', 'fence', 'vegetation', 'pole', 'car', 'sign', 'pedestrian', 'cyclist'};
	opts.train.classesNames = {'back', 'object'};
	colorMap  = (1/255)*[		    
					    128 128 128
					    128 0 0
					    128 64 128
...
colorMap1  = (1/255)*[		    
	...
	opts.train.colorMapEst = colorMap1;

	CLASSES = 2; numel(opts.train.classesNames);
...
function inputs = getBatch(opts, imdb, batch)
	...
	% mapping classes for sparse data
	L = labels(:,:,1,:);
	W = labels(:,:,2,:);
	W2 = [1.0, 20.0];
