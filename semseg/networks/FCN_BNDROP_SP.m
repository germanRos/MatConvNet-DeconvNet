function [net, info] = FCN_BNDROP_SP(imdb, netF, inpt, varargin)
	%run('../matlab/vlfeat-0.9.20/toolbox/vl_setup.m');
    addpath('tools');
    
	% some common options
	trainer = @cnn_train_dag_seg_adv;
    opts.train.fcn_visualize = @visualize_segmentation_sp;
...
% defining batch policies 
     ...
     bopts_train.maxNumBatches  = 15;
     bmanagerTrain              = BatchManagerBalanced(bopts_train);
     
     bopts_val.mode           = 'balanced';
     bopts_val.imdb           = imdb;
     bopts_val.balanceStruct  = [3, 10];
     bopts_val.maxNumBatches  = 2;
     bmanagerVal              = BatchManagerBalanced(bopts_val);
...
%      bmanagerVal                = BatchManagerSeq(bopts_val);
	...
	net.addLayer('prob', dagnn.SoftMax(), {'bigscore'}, {'probx'}, {});
    net.addLayer('sp', dagnn.SP(), {'probx', 'segms'}, {'probx2'}, {});
    net.addLayer('renorm', dagnn.Renormalization(), {'probx2'}, {'prob'}, {});
...
function inputs = getBatch(opts, imdb, batch)
    ...
    %count = 0;
    A = cell(numel(batch), 2000);
	for i=1:numel(batch)
		im = im2single(images(:,:,:,i));
        s = super_pixels(im);
        segms(:,:,:,i) = s;
        %count = count + max(s(:)) + 1;
	...
	if opts.useGpu > 0
		...
		segms = gpuArray(segms);
	end

	inputs = {'input', images, 'label', labels, 'segms', A} ;
end

function initNetF(net, netF)
    net.initParams();
    
    for i=1:numel(netF.params)
        name = netF.params(i).name;
        net.params(net.getParamIndex(name)).value = netF.params(i).value;
    end
end
