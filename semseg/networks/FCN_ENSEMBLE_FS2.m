function [net, info] = FCN_ENSEMBLE_FS2(imdb, netF, inpt, varargin)
    
	% some common options
	trainer = @cnn_train_dag_seg_adv;
	opts.train.fcn_visualize = @visualize_segmentation2;
	...
	opts.train.exampleIndices = [trainData(randperm(numel(trainData), K)), valData(randperm(numel(valData), K))];
     ...
     bopts_train.balanceStruct  =  [1, 2; 2, 5; 12, 3];
	...
	% network definition
	net = dagnn.DagNN() ;
	net.addLayer('FCN_D1', dagnn.FrozenNet('pathmodel', '/home/IMLmatuser/Projects/MatconvNet-DeconvNet/semseg/networks/models_FCNBND/net-epoch-228.mat', 'output_point', 'bigscore'), {'inputR'}, {'FCN_D1'});
	net.addLayer('FCN_D2', dagnn.FrozenNet('pathmodel', '/home/IMLmatuser/Projects/MatconvNet-DeconvNet/semseg/networks/models_FCNBND/net-epoch-243.mat', 'output_point', 'bigscore'), {'inputR'}, {'FCN_D2'});	
	net.addLayer('FCN_S', dagnn.FrozenNet('pathmodel', '/media/IMLmatuser/DATA/Results/NewNetworks/FCN_S.mat', 'output_point', 'upsample'), {'input'}, {'FCN_S'});
	net.addLayer('CONCAT', dagnn.ConcatMultiF('dim', 3, 'numFeats', [11, 11, 6]), {'FCN_D1', 'FCN_D2', 'FCN_S'}, {'CONCAT'});

	net.addLayer('conv1', dagnn.Conv('size', [1, 1, 28, 128], 'hasBias', true, 'stride', [1, 1], 'pad', [0 0 0 0]), {'CONCAT'}, {'conv1'},  {'conv1f'  'conv1b'});
...
function inputs = getBatch(opts, imdb, batch)
	...
	if opts.useGpu > 0
		...
		imagesR = gpuArray(images./127.0);
		labels = gpuArray(labels);
	end

	inputs = {'input', images, 'inputR', imagesR, 'label', labels} ;
end
