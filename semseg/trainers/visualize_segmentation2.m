function visualize_segmentation2(stats, imdb, net, opts)
	epoch = stats.epoch;
...
function showImages(imdb, net, opts)
    ...
    % create inputs
    imagesA = imdb.images.data(:,:,:, indices) ;
	labels = imdb.images.labels(:, :, 1, indices) ;
	if numel(opts.gpus) >  0
		images = gpuArray(imagesA);
  		imagesR = gpuArray(imagesA./127.0);
        net.move('gpu') ;
    end
    ...
    net.eval({'input', images, 'inputR', imagesR});
