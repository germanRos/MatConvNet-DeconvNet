function visualize_segmentationCity(stats, imdb, net, opts)
	epoch = stats.epoch;
...
function showImages(imdb, net, opts)
	...
	D = imdb.images.data(indices);
    	L = imdb.images.labels(indices);

	RESOLUTION = [512, 1024];

	images = zeros(RESOLUTION(1), RESOLUTION(2), 3, numel(indices), 'single');
	imagesorg = zeros(RESOLUTION(1), RESOLUTION(2), 3, numel(indices), 'single');
    labels = ones(RESOLUTION(1), RESOLUTION(2), 2, numel(indices), 'single');
	...
	for i=1:length(indices)
		...
		imshow(squeeze(labels(:,:,1,i))+1, colorMapGT);
		title('Ground truth');
	end
	refreshdata;
	drawnow;
