classdef ConcatMultiF < dagnn.ElementWise
  properties
    dim = 3;
    numFeats = [];
  end
  ...
  methods
    function outputs = forward(obj, inputs, params)
      outputs{1} = vl_nnconcatMultiFeat(inputs, obj.dim, obj.numFeats) ;
      obj.inputSizes = cellfun(@size, inputs, 'UniformOutput', false) ;
    end

    function [derInputs, derParams] = backward(obj, inputs, params, derOutputs)
      derInputs = vl_nnconcatMultiFeat(inputs, obj.dim, obj.numFeats, derOutputs{1}) ;
      derParams = {} ;
    end
    ...
    function obj = ConcatMultiF(varargin)
      obj.load(varargin{:}) ;
    end
  end
end
